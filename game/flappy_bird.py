...
class FlappyBird:
    ...
    def reset(self):
        ...
        first_pipe['x'] = self.WIDTH * 0.6  # Place first pipe at 60% of screen
    ...
    def _get_state(self):
        ...
        if closest_pipe:
            state = np.array([
                ...
                (closest_pipe['x'] - self.bird_x) / self.WIDTH  # Normalized distance to pipe
            ])
        else:
            state = np.array([
    ...
    def _check_collision(self, pipe):
        ...
        if (self.bird_x + self.bird_size > pipe['x'] and 
            self.bird_x < pipe['x'] + 50):  # 50 is pipe width
    ...
    def render(self):
        ...
        for pipe in self.pipes:
            ...
            # Draw bottom pipe
            pygame.draw.rect(self.screen, self.GREEN,
                           (pipe['x'], pipe['gap_y'] + self.PIPE_GAP//2,
                            50, self.HEIGHT - (pipe['gap_y'] + self.PIPE_GAP//2)))
